...
| `youtubeUrl` | Full YouTube link (for video tiles). Leave out if using an image |
| `imageUrl` | Still image path, e.g. `"/assets/my-shot.jpg"` |
| `loopStartMinute` | Tile loop start (minutes) — video only |
| `loopEndMinute` | Tile loop end (minutes). Use `0` for full video |
| `span` | `"normal"`, `"wide"`, or `"tall"` |

### Image project example

1. Upload the file to the `assets/` folder in this repo  
2. Add a project like:
...
```json
{
  ...
  "year": "2026",
  "category": "rigging",
  "overview": "Still turnaround sheet.",
  "tools": ["Maya", "Photoshop"],
  "imageUrl": "/assets/character-turnaround.jpg",
  "span": "normal"
}
```

(No `youtubeUrl` needed for image tiles.)
...
Project stills go in **`assets/`** and are referenced as `"/assets/filename.jpg"`.
